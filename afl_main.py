...
def average_weights(args, models, weights):
    w_avg = deepcopy(models[0])

    for key in w_avg.keys():
        w_avg[key] = torch.zeros_like(w_avg[key])
        for i in range(len(models)):
            w_avg[key] += models[i][key]*weights[i]
...
def main(r):
    ...
    weights = np.array([len(train_loaders[i].dataset) for i in range(args.num_users)])
    ...
    for epoch in range(args.epochs):
        ...
        lambda_ = project(np.array(lambda_) + step_size_lambda * np.array(local_losses))
        # print(lambda_)
        global_weights, client_weights = average_weights(args, local_weights, lambda_)
