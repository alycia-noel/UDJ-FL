...
def average_weights(args, models, local_losses, weights):
    weights_term = [np.exp(args.alpha * local_losses[i]) * weights[i] for i in range(args.num_users)]
    weights_term = list(weights_term / np.sum(weights_term))
    w_avg = deepcopy(models[0])

    for key in w_avg.keys():
        w_avg[key] = torch.zeros_like(w_avg[key])
        for i in range(len(models)):
            w_avg[key] += models[i][key]*weights_term[i]
...
def main(r):
    ...
    client_weights = np.array([len(train_loaders[i].dataset) for i in range(args.num_users)])
    client_weights = list(client_weights / np.sum(client_weights))
    ...
    for epoch in range(args.epochs):
        ...
        global_weights, client_weights = average_weights(args, local_weights, local_losses, client_weights)
