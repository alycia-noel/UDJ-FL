...
def add_models(model1, model2, alpha=1.0):
    ...
    for i, key in enumerate(addition.keys()):
        param1, param2 = model_to_params(model1)[i], model_to_params(model2)[i] 
        with torch.no_grad():
            addition[key] = param1 + alpha * param2
    return addition
...
def main(r):
    ...
    for epoch in range(args.epochs):
        old_model = deepcopy(global_weights)
        ...
        delta_w = [sub_models(old_model, model) for model in local_weights]    # compute pseudo-gradient
        norms = [torch.sqrt(norm2_model(delta)) for delta in delta_w]     # compute pseudo-grad norms
        # normalized pseudo-gradients
        delta_w = [scale_model(delta_w[i], 1 / norms[i]) for i in range(args.num_users)]
        lambda_star = list(solve_centered_w(delta_w, epsilon=args.fedmgda_epsilon))
        avg_delta = avg_models(delta_w, weights=lambda_star)   # d_t as in FedMGDA+
        new_model = add_models(old_model, avg_delta, alpha=-args.global_lr)
