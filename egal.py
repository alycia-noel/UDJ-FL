...
def power_model(model, power):
    print(power)
    powered = deepcopy(model)
    model_params = model_to_params(model)
    for i, key in enumerate(powered.keys()):
        powered[key] = torch.pow(model_params[i], power)
    return powered 
...
def main(r):
    ...
    if args.dataset == 'dirtymnist':
        client_aus = [0.033, .061, .117, .189, .234]
    elif args.dataset == 'curetsr':
        client_aus = [0.357, .492, .5, .824, .913]
    ...
    for epoch in range(args.epochs):
        ...
        if args.inv == 'not_inverse':
            if args.scaled == 'not_scaled':
                g=[a for a in client_aus]
            else:
                g=[a/sum(client_aus) for a in client_aus]
        elif args.inv == 'inverse':
            if args.scaled == 'not_scaled':
                g=[1/a for a in client_aus]
            else:
                inv_au = [1/a for a in client_aus]
                g=[a/sum(inv_au) for a in inv_au]
        elif args.inv == 'gamma_zero':
            g = [1/args.num_users for i in range(args.num_users)]
        ...
        upper = [g[i]*(-args.r*args.beta)*(local_losses[i]**((-args.r*args.beta)-1)) for i in range(args.num_users)]

        Delta = [scale_model(delta_w[i], upper[i]) for i in range(len(delta_w))]

        A = [g[i]*(-args.r*args.beta) for i in range(args.num_users)]
        B = [Lipschitz * (local_losses[i] ** ((-args.r*args.beta) - 1)) for i in range(args.num_users)]
        C = [(norm2_model(delta_w[i]) * (((-args.r*args.beta)-1)*(local_losses[i]**((-args.r*args.beta)-2)))).item() for i in range(args.num_users)]
  
        h = np.multiply(np.multiply(A, np.add(B, C)), np.sign(-args.r*(1+args.r*args.beta)))
        ...
        print(f'Client Acc STD: {np.std(after_agg_test_acc):.4f}')
...
if __name__ == '__main__':
    ...
    print('------------------------------')
    print('Client Test Acc:', all_test_accs, std_client_accs)
    print('Global Acc:', final_global_accs, std_client_full_test_accs)
